"""Poll Datadog API for events/alerts."""
...
async def main(queue: asyncio.Queue, args: dict[str, Any]) -> None:
    """Poll Datadog for alerts and forward to the EDA rulebook."""
        ...
        try:
            ...
            for item in resp.json().get("data", resp.json() if isinstance(resp.json(), list) else []):
                ...
                if item_id and item_id not in seen:
                    seen.add(item_id)
                    await queue.put(dict([("datadog", item)]))
        except Exception as exc:
            logger.error("Error polling Datadog: %s", exc)
