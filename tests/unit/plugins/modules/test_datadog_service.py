"""Unit tests for stevefulme1.datadog.datadog_service module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.datadog.plugins.modules.datadog_service"
CLIENT_PATH = "ansible_collections.stevefulme1.datadog.plugins.module_utils.api_client"


@pytest.fixture
def mock_api_client():
    """Mock API client for datadog_service."""
    client = MagicMock()
    client.get.return_value = None
    client.create.return_value = {"service_name": "res-123", "name": "test-service"}
    client.update.return_value = {"service_name": "res-123", "name": "test-service-updated"}
...
def existing_resource():
    """Return a dict representing an existing service."""
    return {
        "service_name": "res-123",
        "name": "test-service",
        "state": "active",
    }
...
class TestCreateService:
    ...
    def test_create_returns_resource(self, mock_api_client):
        ...
        result = mock_api_client.create("service", {"name": "test-service"})
        assert result["service_name"] == "res-123"
    ...
    def test_create_with_all_params(self, mock_api_client):
        ...
        mock_api_client.create("service", params)
        mock_api_client.create.assert_called_once_with("service", params)
    ...
    def test_create_api_error(self):
        ...
        with pytest.raises(Exception, match="409 Conflict"):
            client.create("service", {"name": "dup"})
    ...
    def test_create_check_mode_no_api_call(self, mock_api_client):
        ...
        if check_mode:
            ...
            result = mock_api_client.create("service", {})
...
class TestUpdateService:
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("service", "res-123", {"name": "updated"})
        assert result["name"] == "test-service-updated"
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("service", "bad-id", {})
...
class TestDeleteService:
    ...
    def test_delete_existing(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("service", "res-123")
        mock_api_client.delete.assert_called_once_with("service", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("service", "missing-id")
...
class TestGetService:
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("service", "res-123")


class TestListService:
    """Tests for listing service resources."""

    def test_list_returns_all(self, mock_api_client):
        """Verify list returns all resources."""
        mock_api_client.list.return_value = [
            {"service_name": "1", "name": "first"},
            {"service_name": "2", "name": "second"},
        ]
        result = mock_api_client.list("service")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [{"service_name": "1", "name": "match"}]
        result = mock_api_client.list("service", filters={"name": "match"})
        assert len(result) == 1


class TestIdempotencyService:
    """Tests for idempotent behavior of service."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("service", "res-123")
    ...
    def test_delete_absent_is_idempotent(self, mock_api_client):
        ...
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("service", "missing") is not None
