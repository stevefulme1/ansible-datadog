"""Unit tests for stevefulme1.datadog.datadog_metric_metadata module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.datadog.plugins.modules.datadog_metric_metadata"
...
class TestCreateMetricMetadata:
    """Tests for creating a metric_metadata."""
    ...
    def test_create_with_all_params(self, mock_api_client):
        ...
        params = {
            "metric_name": "full-metric_metadata",
            "description": "Full test",
            "tags": {"env": "test"},
        }
        mock_api_client.create("metric_metadata", params)
        mock_api_client.create.assert_called_once_with("metric_metadata", params)
    ...
    def test_create_api_error(self):
        ...
        with pytest.raises(Exception, match="409 Conflict"):
            client.create("metric_metadata", {"metric_name": "dup"})

    def test_create_check_mode_no_api_call(self, mock_api_client):
        """Verify check_mode skips actual API call."""
        check_mode = True
        if check_mode:
            result = {"changed": True, "metric_metadata": {}}
        else:
            result = mock_api_client.create("metric_metadata", {})
        assert result["changed"] is True
        mock_api_client.create.assert_not_called()


class TestUpdateMetricMetadata:
    """Tests for updating a metric_metadata."""
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("metric_metadata", "res-123", {"metric_name": "updated"})
        assert result["metric_name"] == "test-metric_metadata-updated"
    ...
    def test_update_idempotent_no_change(self, mock_api_client, existing_resource):
        ...
        desired = {"metric_name": existing_resource["metric_name"]}
        current = {"metric_name": existing_resource["metric_name"]}
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("metric_metadata", "bad-id", {})


class TestDeleteMetricMetadata:
    """Tests for deleting a metric_metadata."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("metric_metadata", "res-123")
        mock_api_client.delete.assert_called_once_with("metric_metadata", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("metric_metadata", "missing-id")
    ...
    def test_delete_api_error(self):
        ...
        with pytest.raises(Exception, match="403 Forbidden"):
            client.delete("metric_metadata", "res-123")


class TestGetMetricMetadata:
    """Tests for getting a metric_metadata."""
    ...
    def test_get_nonexistent(self, mock_api_client):
        ...
        mock_api_client.get.return_value = None
        result = mock_api_client.get("metric_metadata", "nonexistent")
        assert result is None
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("metric_metadata", "res-123")


class TestListMetricMetadata:
    """Tests for listing metric_metadata resources."""
...
class TestIdempotencyMetricMetadata:
    """Tests for idempotent behavior of metric_metadata."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("metric_metadata", "res-123")
        desired_params = {"metric_name": current["metric_name"]}
        # If resource exists and matches desired state, no change
        changed = desired_params["metric_name"] != current["metric_name"]
        assert changed is False

    def test_delete_absent_is_idempotent(self, mock_api_client):
        """Verify deleting an absent resource reports no change."""
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("metric_metadata", "missing") is not None
        assert exists is False


class TestErrorHandlingMetricMetadata:
    """Tests for error handling in metric_metadata."""
