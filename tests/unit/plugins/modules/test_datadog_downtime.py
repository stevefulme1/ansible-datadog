"""Unit tests for stevefulme1.datadog.datadog_downtime module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.datadog.plugins.modules.datadog_downtime"
CLIENT_PATH = "ansible_collections.stevefulme1.datadog.plugins.module_utils.api_client"


@pytest.fixture
def mock_api_client():
    """Mock API client for datadog_downtime."""
    client = MagicMock()
    client.get.return_value = None
    client.create.return_value = {"downtime_id": "res-123", "scope": "test-downtime"}
    client.update.return_value = {"downtime_id": "res-123", "scope": "test-downtime-updated"}
...
def existing_resource():
    """Return a dict representing an existing downtime."""
    return {
        "downtime_id": "res-123",
        "scope": "test-downtime",
        "state": "active",
...
class TestCreateDowntime:
    """Tests for creating a downtime."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("downtime", {"scope": "test-downtime"})
        assert result["downtime_id"] == "res-123"
        assert result["scope"] == "test-downtime"
    ...
    def test_create_with_all_params(self, mock_api_client):
        ...
        mock_api_client.create("downtime", params)
        mock_api_client.create.assert_called_once_with("downtime", params)
    ...
    def test_create_api_error(self):
        ...
        with pytest.raises(Exception, match="409 Conflict"):
            client.create("downtime", {"scope": "dup"})

    def test_create_check_mode_no_api_call(self, mock_api_client):
        """Verify check_mode skips actual API call."""
        check_mode = True
        if check_mode:
            result = {"changed": True, "downtime": {}}
        else:
            result = mock_api_client.create("downtime", {})
        assert result["changed"] is True
        mock_api_client.create.assert_not_called()


class TestUpdateDowntime:
    """Tests for updating a downtime."""
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("downtime", "res-123", {"scope": "updated"})
        assert result["scope"] == "test-downtime-updated"
    ...
    def test_update_idempotent_no_change(self, mock_api_client, existing_resource):
        ...
        desired = {"scope": existing_resource["scope"]}
        current = {"scope": existing_resource["scope"]}
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("downtime", "bad-id", {})


class TestDeleteDowntime:
    """Tests for deleting a downtime."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("downtime", "res-123")
        mock_api_client.delete.assert_called_once_with("downtime", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("downtime", "missing-id")
...
class TestGetDowntime:
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("downtime", "res-123")


class TestListDowntime:
    """Tests for listing downtime resources."""

    def test_list_returns_all(self, mock_api_client):
        """Verify list returns all resources."""
        mock_api_client.list.return_value = [
            {"downtime_id": "1", "scope": "first"},
            {"downtime_id": "2", "scope": "second"},
        ]
        result = mock_api_client.list("downtime")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [{"downtime_id": "1", "scope": "match"}]
        result = mock_api_client.list("downtime", filters={"scope": "match"})
        assert len(result) == 1


class TestIdempotencyDowntime:
    """Tests for idempotent behavior of downtime."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("downtime", "res-123")
        desired_params = {"scope": current["scope"]}
        # If resource exists and matches desired state, no change
        changed = desired_params["scope"] != current["scope"]
        assert changed is False

    def test_delete_absent_is_idempotent(self, mock_api_client):
        """Verify deleting an absent resource reports no change."""
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("downtime", "missing") is not None
        assert exists is False


class TestErrorHandlingDowntime:
    """Tests for error handling in downtime."""
