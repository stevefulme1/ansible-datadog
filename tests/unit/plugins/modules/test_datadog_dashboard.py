"""Unit tests for stevefulme1.datadog.datadog_dashboard module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.datadog.plugins.modules.datadog_dashboard"
CLIENT_PATH = "ansible_collections.stevefulme1.datadog.plugins.module_utils.api_client"


@pytest.fixture
def mock_api_client():
    """Mock API client for datadog_dashboard."""
    client = MagicMock()
    client.get.return_value = None
    client.create.return_value = {"dashboard_id": "res-123", "title": "test-dashboard"}
    client.update.return_value = {"dashboard_id": "res-123", "title": "test-dashboard-updated"}
...
class TestCreateDashboard:
    ...
    def test_create_returns_resource(self, mock_api_client):
        ...
        result = mock_api_client.create("dashboard", {"title": "test-dashboard"})
    ...
    def test_create_api_error(self):
        ...
        with pytest.raises(Exception, match="409 Conflict"):
            client.create("dashboard", {"title": "dup"})
...
class TestUpdateDashboard:
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("dashboard", "res-123", {"title": "updated"})
        assert result["title"] == "test-dashboard-updated"
    ...
    def test_update_idempotent_no_change(self, mock_api_client, existing_resource):
        ...
        desired = {"title": existing_resource["title"]}
        current = {"title": existing_resource["title"]}
...
class TestListDashboard:
    ...
    def test_list_returns_all(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [
            {"dashboard_id": "1", "title": "first"},
            {"dashboard_id": "2", "title": "second"},
        ]
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [{"dashboard_id": "1", "title": "match"}]
        result = mock_api_client.list("dashboard", filters={"title": "match"})
        assert len(result) == 1
...
class TestIdempotencyDashboard:
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        desired_params = {"title": current["title"]}
        # If resource exists and matches desired state, no change
        changed = desired_params["title"] != current["title"]
        assert changed is False
