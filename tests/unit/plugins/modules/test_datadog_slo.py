"""Unit tests for stevefulme1.datadog.datadog_slo module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.datadog.plugins.modules.datadog_slo"
CLIENT_PATH = "ansible_collections.stevefulme1.datadog.plugins.module_utils.api_client"


@pytest.fixture
def mock_api_client():
    """Mock API client for datadog_slo."""
    client = MagicMock()
    client.get.return_value = None
    client.create.return_value = {"slo_id": "res-123", "name": "test-slo"}
    client.update.return_value = {"slo_id": "res-123", "name": "test-slo-updated"}
...
def existing_resource():
    """Return a dict representing an existing slo."""
    return {
        "slo_id": "res-123",
        "name": "test-slo",
        "state": "active",
...
class TestCreateSlo:
    """Tests for creating a slo."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("slo", {"name": "test-slo"})
        assert result["slo_id"] == "res-123"
        assert result["name"] == "test-slo"
    ...
    def test_create_with_all_params(self, mock_api_client):
        ...
        mock_api_client.create("slo", params)
        mock_api_client.create.assert_called_once_with("slo", params)
...
class TestUpdateSlo:
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("slo", "res-123", {"name": "updated"})
        assert result["name"] == "test-slo-updated"
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("slo", "bad-id", {})


class TestDeleteSlo:
    """Tests for deleting a slo."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("slo", "res-123")
        mock_api_client.delete.assert_called_once_with("slo", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("slo", "missing-id")
...
class TestGetSlo:
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("slo", "res-123")


class TestListSlo:
    """Tests for listing slo resources."""

    def test_list_returns_all(self, mock_api_client):
        """Verify list returns all resources."""
        mock_api_client.list.return_value = [
            {"slo_id": "1", "name": "first"},
            {"slo_id": "2", "name": "second"},
        ]
        result = mock_api_client.list("slo")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [{"slo_id": "1", "name": "match"}]
        result = mock_api_client.list("slo", filters={"name": "match"})
        assert len(result) == 1


class TestIdempotencySlo:
    """Tests for idempotent behavior of slo."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("slo", "res-123")
    ...
    def test_delete_absent_is_idempotent(self, mock_api_client):
        ...
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("slo", "missing") is not None
        assert exists is False


class TestErrorHandlingSlo:
    """Tests for error handling in slo."""
