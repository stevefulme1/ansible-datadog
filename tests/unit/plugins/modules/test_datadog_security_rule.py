"""Unit tests for stevefulme1.datadog.datadog_security_rule module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.datadog.plugins.modules.datadog_security_rule"
CLIENT_PATH = "ansible_collections.stevefulme1.datadog.plugins.module_utils.api_client"


@pytest.fixture
def mock_api_client():
    """Mock API client for datadog_security_rule."""
    client = MagicMock()
    client.get.return_value = None
    client.create.return_value = {"rule_id": "res-123", "name": "test-security_rule"}
    client.update.return_value = {"rule_id": "res-123", "name": "test-security_rule-updated"}
...
def existing_resource():
    """Return a dict representing an existing security_rule."""
    return {
        "rule_id": "res-123",
        "name": "test-security_rule",
        "state": "active",
    }


class TestCreateSecurityRule:
    """Tests for creating a security_rule."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("security_rule", {"name": "test-security_rule"})
        assert result["rule_id"] == "res-123"
        assert result["name"] == "test-security_rule"
        mock_api_client.create.assert_called_once()

    def test_create_with_all_params(self, mock_api_client):
        """Verify create passes all parameters to API."""
        params = {
            "name": "full-security_rule",
            "description": "Full test",
            "tags": {"env": "test"},
        }
        mock_api_client.create("security_rule", params)
        mock_api_client.create.assert_called_once_with("security_rule", params)
...
class TestUpdateSecurityRule:
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("security_rule", "res-123", {"name": "updated"})
        assert result["name"] == "test-security_rule-updated"
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("security_rule", "bad-id", {})


class TestDeleteSecurityRule:
    """Tests for deleting a security_rule."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("security_rule", "res-123")
        mock_api_client.delete.assert_called_once_with("security_rule", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("security_rule", "missing-id")
    ...
    def test_delete_api_error(self):
        ...
        with pytest.raises(Exception, match="403 Forbidden"):
            client.delete("security_rule", "res-123")


class TestGetSecurityRule:
    """Tests for getting a security_rule."""

    def test_get_existing(self, mock_api_client, existing_resource):
        """Verify get returns resource when it exists."""
        mock_api_client.get.return_value = existing_resource
        result = mock_api_client.get("security_rule", "res-123")
        assert result["rule_id"] == "res-123"

    def test_get_nonexistent(self, mock_api_client):
        """Verify get returns None for missing resource."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("security_rule", "nonexistent")
        assert result is None
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("security_rule", "res-123")


class TestListSecurityRule:
    """Tests for listing security_rule resources."""

    def test_list_returns_all(self, mock_api_client):
        """Verify list returns all resources."""
        mock_api_client.list.return_value = [
            {"rule_id": "1", "name": "first"},
            {"rule_id": "2", "name": "second"},
        ]
        result = mock_api_client.list("security_rule")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [{"rule_id": "1", "name": "match"}]
        result = mock_api_client.list("security_rule", filters={"name": "match"})
        assert len(result) == 1


class TestIdempotencySecurityRule:
    """Tests for idempotent behavior of security_rule."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("security_rule", "res-123")
    ...
    def test_delete_absent_is_idempotent(self, mock_api_client):
        ...
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("security_rule", "missing") is not None
        assert exists is False


class TestErrorHandlingSecurityRule:
    """Tests for error handling in security_rule."""
