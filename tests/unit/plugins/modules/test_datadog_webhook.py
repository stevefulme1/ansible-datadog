"""Unit tests for stevefulme1.datadog.datadog_webhook module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.datadog.plugins.modules.datadog_webhook"
CLIENT_PATH = "ansible_collections.stevefulme1.datadog.plugins.module_utils.api_client"


@pytest.fixture
def mock_api_client():
    """Mock API client for datadog_webhook."""
    client = MagicMock()
    client.get.return_value = None
    client.create.return_value = {"webhook_name": "res-123", "name": "test-webhook"}
    client.update.return_value = {"webhook_name": "res-123", "name": "test-webhook-updated"}
...
def existing_resource():
    """Return a dict representing an existing webhook."""
    return {
        "webhook_name": "res-123",
        "name": "test-webhook",
        "state": "active",
    }
...
class TestCreateWebhook:
    """Tests for creating a webhook."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("webhook", {"name": "test-webhook"})
        assert result["webhook_name"] == "res-123"
    ...
    def test_create_with_all_params(self, mock_api_client):
        ...
        mock_api_client.create("webhook", params)
        mock_api_client.create.assert_called_once_with("webhook", params)
...
class TestUpdateWebhook:
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("webhook", "res-123", {"name": "updated"})
        assert result["name"] == "test-webhook-updated"
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("webhook", "bad-id", {})


class TestDeleteWebhook:
    """Tests for deleting a webhook."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("webhook", "res-123")
        mock_api_client.delete.assert_called_once_with("webhook", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("webhook", "missing-id")
...
class TestGetWebhook:
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("webhook", "res-123")


class TestListWebhook:
    """Tests for listing webhook resources."""

    def test_list_returns_all(self, mock_api_client):
        """Verify list returns all resources."""
        mock_api_client.list.return_value = [
            {"webhook_name": "1", "name": "first"},
            {"webhook_name": "2", "name": "second"},
        ]
        result = mock_api_client.list("webhook")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [{"webhook_name": "1", "name": "match"}]
        result = mock_api_client.list("webhook", filters={"name": "match"})
        assert len(result) == 1


class TestIdempotencyWebhook:
    """Tests for idempotent behavior of webhook."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("webhook", "res-123")
    ...
    def test_delete_absent_is_idempotent(self, mock_api_client):
        ...
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("webhook", "missing") is not None
        assert exists is False


class TestErrorHandlingWebhook:
